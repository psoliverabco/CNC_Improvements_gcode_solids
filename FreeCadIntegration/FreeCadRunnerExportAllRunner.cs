...
{
    internal static class FreeCadRunnerExportAllRunner
    {
        public static string SaveScript()
        {
            string scriptText = FreeCadScriptExportAll.BuildScriptText();
            ...
            int n = FreeCadRunSuffix.NextMerge();
            string scriptPath = Path.Combine(scriptDir, $"npc_merge_steps_{n:000}.py");
        ...
        public static string RunFreeCad(string scriptPath, string workingDirectory)
        {
            if (string.IsNullOrWhiteSpace(scriptPath) || !File.Exists(scriptPath))
                throw new FileNotFoundException("ExportAll merge Python script not found.", scriptPath);
