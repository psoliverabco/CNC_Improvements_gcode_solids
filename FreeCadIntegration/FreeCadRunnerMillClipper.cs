...
{
    internal static class FreeCadRunnerMillClipper
    {
        public static string SaveScript(string stepPath)
        {
            ...
            // ✅ Correct class + correct sentinel
            if (string.IsNullOrWhiteSpace(FreeCadScriptMillClipper.MillShapeClipper) ||
                FreeCadScriptMillClipper.MillShapeClipper.IndexOf("Not yet set MillShapeClipper", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                throw new InvalidOperationException("FreeCadScriptMillClipper.MillShapeClipper has not been initialized.");
            }
            ...
            int n = FreeCadRunSuffix.NextMillClipper();

            // (Optional) less confusing name:
            // string scriptPath = Path.Combine(scriptDir, $"npc_mill_clipper_{n:000}.py");
            string scriptPath = Path.Combine(scriptDir, $"npc_mill_clipper_{n:000}.py");

            // ✅ If you want this driven by your existing RemoveSplitter option:
            // (True => call removeSplitter at end)
            ...
            // Use your existing UI boolean (or set a fixed default)
            string cutRemoveSplitter = PyBool(FreeCadScriptMill.RemoveSplitter, "False");

            string optionsBlock =
                "\n# ------------------------------------------------------------\n" +
                "# OPTIONS (from CNC_Improvements_gcode_solids)\n" +
                $"CutRemoveSplitterAtEnd = {cutRemoveSplitter}\n\n";

            // ✅ MUST be CLIPPER_TEXT (TailPY expects this)
            string clipperBlock =
                "CLIPPER_TEXT = \"\"\"\n" +
                FreeCadScriptMillClipper.MillShapeClipper +
                "\n\"\"\"\n";

            string scriptText =
                $"output_step = r\"{stepPath}\"\n" +
                FreeCadScriptMillClipper.HeadPY +
                optionsBlock +
                clipperBlock +
                FreeCadScriptMillClipper.TransPY +
                FreeCadScriptMillClipper.TailPY;
